...
train_dir=`pwd`/../kaldi-trunk3/egs/wsj/s5_test/data/train_si284 # clean directory
train_noise_dir=$train_dir/../train_si284_noise_snr0-8 # noise directory
wav_file=$train_dir/wav.scp
if [ 1 == 1 ]; then
[ ! -d $train_noise_dir ] && mkdir $train_noise_dir 
cp $train_dir/* $train_noise_dir/
...
mv $train_noise_dir/utt2dur $train_noise_dir/utt2dur.old
...
echo "generate utt2dur in $train_noise_dir"
sort -k 2 $train_noise_dir/utt2spk | $train_dir/../../utils/utt2spk_to_spk2utt.pl > $train_noise_dir/spk2utt 
cat $train_noise_dir/utt2dur.old | awk '{for(i=1;i<=NF;i++){printf "%s ",$i};printf "\n";printf "%s-noise ",$1;for(i=2;i<=NF;i++){printf "%s ",$i};printf "\n"}' > $train_noise_dir/utt2dur
