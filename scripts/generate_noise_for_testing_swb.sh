...
#test_noise_dir=$test_dir/../eval2000_noise # noise directory
...
while read p; do
    ...
    echo "${uid}-noise $wav python $d_cmd -r 8000 -c noise[filter=$noise,snr=$snr] stdin.wav stdout.wav |" >> $test_noise_dir/wav.scp
done < $wav_file

echo "generate text"
cat $test_noise_dir/text.old | awk '{for(i=1;i<=NF;i++){printf "%s ",$i};printf "\n";printf "%s-noise ",$1;for(i=2;i<=NF;i++){printf "%s ",$i};printf "\n"}' > $test_noise_dir/text 

echo "generate segments in $test_noise_dir"
cat $test_noise_dir/segments.old | awk '{for(i=1;i<=NF;i++){printf "%s ",$i};printf "\n";printf "%s-noise ",$1;printf "%s-noise ",$2;for(i=3;i<=NF;i++){printf "%s ",$i};printf "\n"}' > $test_noise_dir/segments

echo "generate reco2file_and_channel"
cat $test_noise_dir/reco2file_and_channel.old | awk '{for(i=1;i<=NF;i++){printf "%s ",$i};printf "\n";printf "%s-noise ",$1;for(i=2;i<=NF;i++){printf "%s ",$i};printf "\n"}' > $test_noise_dir/reco2file_and_channel
