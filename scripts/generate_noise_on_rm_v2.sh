...
test_noise_dir=$noise_dir/data/test_ukn # noise directory
test_noise_dir2=$noise_dir/data/test_kn # noise directory
...
[ -d $test_noise_dir2 ] && rm -rf $test_noise_dir2
mkdir -p $test_noise_dir2 
...
if [ $stage -le 2 ]; then
	...
	less -c $test_noise_dir2/wav.scp | awk '{spk=substr($1,1,4);printf("%s %s\n", $1, spk)}' | sort -u > $test_noise_dir2/utt2spk
